...
export default function NavBar() {
  ...
  const NAVBAR_CONTROLS = {
    ...
    navbarPadding: "py-1", // Navbar height: 'py-1' (smaller), 'py-6' (taller)
    ...
    logoSize: "width={55} height={55}", // Logo image size (change both width and height)
  ...
  return (
    <header className="sticky top-0 z-30 bg-sentra-travertine/90 backdrop-blur border-b border-sentra-dune-mist">
      <nav
        className={`mx-auto flex max-w-7xl items-center justify-between ${NAVBAR_CONTROLS.navbarPadding} ${NAVBAR_CONTROLS.navbarHorizontal}`}
      >
        {/* logo block */}
        <Link
          href="#intro"
          className={`flex items-center ${NAVBAR_CONTROLS.logoGap}`}
        >
          <Image src="/logo.png" width={55} height={55} alt="Sentra logo" />
          <span
            className={`${NAVBAR_CONTROLS.logoTextSize}  font-rockwell text-sentra-midnight-deck`}
          >
            Sentra
          </span>
        </Link>
        ...
        <div className={`flex items-center ${NAVBAR_CONTROLS.navToButtonGap}`}>
          <ul
            className={`hidden md:flex items-center ${NAVBAR_CONTROLS.navLinksGap} text-sentra-midnight-deck`}
          >
            {links.map((l) => (
              <li key={l.href}>
                <Link
                  href={l.href}
                  className="hover:text-sentra-apricot-jet transition-colors"
                >
                  {l.label}
                </Link>
              </li>
