...
export default function IntroSection() {
  ...
  const [paused, setPaused] = useState(true); // true when hovering
  const [lastHoveredIdx, setLastHoveredIdx] = useState(0); // track last hovered for sticky behavior
  ...
  const MANUAL_CONTROLS = {
    ...
    descriptionSize: "text-xl md:text-3xl lg:text-3xl", // Description text size: 'text-base md:text-xl lg:text-2xl' (smaller), 'text-xl md:text-3xl lg:text-4xl' (larger)
    ...
    answerLineHeight: "leading-relaxed", // Controls line spacing for the answer sentence e.g. leading-snug, leading-normal, leading-loose
  };
  ...
  return (
    <section id="intro" className="relative isolate h-screen">
      <div className="h-full grid lg:grid-cols-2 items-start lg:items-center">
        {/* LEFT: copy block */}
        <div
          className={`mx-auto max-w-7xl px-6 lg:pl-8 w-full ${MANUAL_CONTROLS.leftWidth}`}
        >
          <div
            className={`${MANUAL_CONTROLS.contentPadding} ${MANUAL_CONTROLS.leftVertical} ${MANUAL_CONTROLS.leftHorizontal}`}
          >
            <h1
              className={`${MANUAL_CONTROLS.titleSize} text-sentra-midnight-deck ${MANUAL_CONTROLS.titleMargin} font-display font-semibold`}
            >
              What is Sentra?
            </h1>
            ...
            >
              {/* Line 1 - now a single flowing paragraph */}
              <p
                className={`${MANUAL_CONTROLS.answerLineHeight} font-rockwell`}
              >
                <span className="font-semibold">Sentra is a </span>
                <Keyword
                  word={tokens[0].word}
                  index={0}
                  active={activeIdx}
                ...
                <span className="font-semibold">
                  travel platform that caters to travel agencies of
                </span>
                {", "}
                <Keyword
                  word={tokens[4].word}
            ...
            <div className="min-h-[12rem] lg:min-h-[16rem] xl:min-h-[22rem]">
              {activeToken && (
                ...
                >
                  <span
                    className={`${activeToken.color} font-bold underline underline-offset-4 decoration-2 ${activeToken.decorationColor} ${MANUAL_CONTROLS.definitionKeywordSize}`}
                  >
                    {activeToken.word}:
                  </span>
                  <p
                    className={`${activeToken.color} ${MANUAL_CONTROLS.definitionSpacing} ${MANUAL_CONTROLS.definitionStyle} leading-relaxed font-inter`}
                  >
        ...
        {/* <div
          className={`flex justify-center lg:justify-end items-center h-full lg:pl-8 ${MANUAL_CONTROLS.rightWidth}`}
        >
          <div
